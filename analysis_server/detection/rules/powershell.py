...
class SuspiciousScriptBlockContentRule(PerEventRule):
    """4104 script block text contains known-malicious patterns."""
...
class OrphanedScriptBlockRule(AggregateRule):
    """
    4105 (started) without matching 4106 (completed) — script block was
    forcefully terminated. Indicator of crash, AV kill, or process injection gone wrong.
    """
...
class ScriptBlockBurstRule(AggregateRule):
    """
    Many unique script blocks invoked within a short window from the same runspace —
    indicator of automated enumeration or stager execution.
    """
