...
from services.collections import list_collections, get_collection_path, delete_collection
...
async def handle_command(msg: dict, conn_state: dict, websocket: WebSocket) -> dict:
    ...
    if conn_state["mode"] == "awaiting_input":
        ...
        if conn_state["flow_steps"]:
            next_step = conn_state["flow_steps"][0]
            return _prompt_payload(next_step)
    ...
    if msg_type == "choice":
        ...
        if conn_state.get("menu") == "collection_detail":
            ...
            elif value == 2:  # Delete: ask for confirmation
